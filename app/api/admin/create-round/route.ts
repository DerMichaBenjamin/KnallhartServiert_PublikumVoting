...
export async function POST(request: NextRequest) {
  ...
  if (!configState.ok) {
    return NextResponse.json({ ok: false, error: configState.message }, { status: 500 });
  }
  ...
  if (Number.isNaN(new Date(startAt).getTime()) || Number.isNaN(new Date(endAt).getTime())) {
    return NextResponse.json({ ok: false, error: 'Start oder Ende ist kein gültiges Datum.' }, { status: 400 });
  }
  if (new Date(startAt).getTime() >= new Date(endAt).getTime()) {
    return NextResponse.json({ ok: false, error: 'Das Enddatum muss nach dem Startdatum liegen.' }, { status: 400 });
  }
